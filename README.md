...
Session is used to store identification data on the server once the user has been authenticated. And it will need to be destroyed on logout. It's one of the solution to retain data between http request since HTTP is stateless. 

1. What does bcrypt do to help us store passwords in a secure manner.
Bcrypt uses one-way hashing by adding a 'salt', a secret that only the server knows, to the encryption process, which can also be specified to run any number of times to combat against the "rainbow table" attack.

1. What does bcrypt do to slow down attackers?
It uses Blowfish cipher has an algorithm which make key changing a slow process, couple that with the variable iterations, making it increasingly slower over time.

1. What are the three parts of the JSON Web Token?
- The payload object
- The secret / salt used
- The option object
